...
impl BigUint {
    ...
    pub fn sub_mod_ex(&self, rhs: &Self, modulo: &Self) -> Self {
        todo!()
    }
}
...
mod tests {
    ...
    fn borrowing_sub_w_carry() {
        let op1 = BigUint::from_u128(0);
        let op2 = BigUint::from_u128(5);
        let mut res_slice = vec![0xFFFF_FFFF - 4];
        res_slice.extend_from_slice(&[0xFFFF_FFFF; BigUint::NUM_LIMBS - 1]);
        assert_eq!(
            op1.borrowing_sub(&op2, 0),
            (BigUint::from_slice(&res_slice), 1)
        );
    }
}
